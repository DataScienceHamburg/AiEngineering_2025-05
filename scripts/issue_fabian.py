...
for epoch in range(EPOCHS):
    ...
    for i, (X_train_batch, y_train_batch) in enumerate(train_loader):
        ...
        # loss calculation
        loss = torch.nn.functional.mse_loss(y_pred, y_train_batch)
    ...
    test_losses_epoch.append(test_losses_epoch)
...
with torch.no_grad():
    ...
    for X_train_batch, y_train_batch in train_loader:
        y_pred.extend(model(X_train_batch).detach().numpy().flatten().tolist())
        y_true.extend(y_train_batch)
 
 
# %% calc correl. coefficient
